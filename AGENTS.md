...
- Stack: Kotlin 2.4.0, Java 17 toolchain, **Kotlin** MCP SDK 0.11.0 (`io.modelcontextprotocol:kotlin-sdk-server`), Gradle Tooling API 9.6.1, kotlinx.serialization（ツール結果 JSON・MCP ワイヤ）。
...
1. Downloads release **v0.3.3** JAR (SHA-256 verified) to `~/.local/share/gradle-tapi-mcp-server/gradle-tapi-mcp-server.jar` — **before** `./gradlew` so MCP can drive this repo's build
...
| Goal | Command |
|------|---------|
| Build JAR | `./gradlew jar` → `build/libs/gradle-tapi-mcp-server-0.3.3.jar` |
...
| Run MCP server | `GRADLE_PROJECT_DIR=/workspace java -jar build/libs/gradle-tapi-mcp-server-0.3.3.jar` |
