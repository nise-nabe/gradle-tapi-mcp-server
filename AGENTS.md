...
- Use `project-context-ingestion` for declared build constraints; reserve Gradle MCP for resolved runtime Gradle/Java, build verification, task execution, and source locate (`artifacts[]` + `downloadSources` for versions not in the project graph). Do not browse `gradleUserHome` / `~/.gradle` to discover versions or sources.
...
1. Downloads release JAR (version in `.cursor/install.sh` `GRADLE_TAPI_MCP_VERSION`, currently **0.11.0**) with SHA-256 verification to `~/.local/share/gradle-tapi-mcp-server/gradle-tapi-mcp-server.jar` so MCP can drive this repo's build when needed
...
| Goal | Command |
|------|---------|
| Build JAR | `./gradlew jar` → `build/libs/gradle-tapi-mcp-server-0.11.0.jar` |
...
| Run MCP server | `GRADLE_PROJECT_DIR=/workspace java -jar build/libs/gradle-tapi-mcp-server-0.11.0.jar` |
