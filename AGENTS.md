...
- Use `project-context-ingestion` for declared build constraints; reserve Gradle MCP for resolved runtime Gradle/Java, build verification, task execution, and source locate (`artifacts[]` + `downloadSources: true` for versions not in the project graph). Do not browse `gradleUserHome` / `~/.gradle` to discover versions or sources.
...
- Agent skill at `plugins/gradle-tapi-mcp/skills/gradle-tapi-mcp/` (install via Cursor/Codex/Copilot marketplace or `devin plugins install nise-nabe/gradle-tapi-mcp-server#plugins/gradle-tapi-mcp`, or copy to `~/.cursor/skills/` globally) documents token-efficient MCP workflows: prefer `gradle_get_project_overview`; use `gradle_get_build_cache_status` for cache settings; task lists omitted unless `includeTasks=true`; run output omitted by default (`includeOutput=false`; outcome/buildSummary only). Dependency-sources index → search → read workflow (with JSON examples) lives in `docs/dependency-sources.md`. Idea keep-set is the current project's attached sources; other versions / Kotlin / plugin GAVs use `artifacts[]` + `downloadSources: true` (do not list Gradle user home).
- Agent skill at `skills/release/` (summary in `.cursor/skills/release/SKILL.md`) documents the GitHub release workflow: version bump PR (including marketplace / plugin `version`), JAR build, tag, Release asset upload, then `install.sh` / `server-release.json` SHA-256.
- MCP server holds a **connection pool** keyed by canonical project path, refcounted per MCP session (`holders` in `GradleConnectionManager`); `gradle_connect` ensures a project without disconnecting others and becomes the session's default project (`SessionProjectContext`). Optional `projectDirectory` on tools resolves to the session default → session-known projects → `GRADLE_PROJECT_DIR`. `gradle_disconnect` without args releases the session default only (`all: true` closes everything); session-scoped views (connection status, build list/status/cancel) never reach other sessions' projects. `X-Gradle-Project-Dir` on the streamable-HTTP initialize binds a project per session.
- Startup auto-connect runs **detached** so `initialize` is never blocked by a first-time Gradle distribution download; `captureProtocolStdout()` (server package) redirects `System.out` (where the Tooling API wrapper installer writes download progress) to stderr so stdio stdout stays JSON-only. `gradle_connection_status` reports `connecting: true` while a connect/download is in flight; `gradle_connect` forwards FILE_DOWNLOAD progress as log notifications and wires request cancellation to the Tooling API `CancellationToken`.
...
1. Downloads release JAR (version in `.cursor/install.sh` `GRADLE_TAPI_MCP_VERSION`, currently **0.14.0**) with SHA-256 verification to `~/.local/share/gradle-tapi-mcp-server/gradle-tapi-mcp-server.jar` so MCP can drive this repo's build when needed
...
The `gradle` MCP server is defined in `.cursor/mcp.json`. Token-efficient workflows: `.cursor/skills/gradle-tapi-mcp/SKILL.md` (summary) and `plugins/gradle-tapi-mcp/skills/gradle-tapi-mcp/` (full reference). Release workflow: `.cursor/skills/release/SKILL.md` (summary) and `skills/release/` (full reference). Marketplace catalogs: `.cursor-plugin/marketplace.json` (Cursor), `.agents/plugins/marketplace.json` (Codex), `.github/plugin/marketplace.json` (GitHub Copilot). Devin has no catalog file — it installs the plugin by git-subdir (`owner/repo#plugins/gradle-tapi-mcp`) using `plugins/gradle-tapi-mcp/.devin-plugin/plugin.json`; do not add a repo-root `.devin-plugin/` meta-plugin (the root `AGENTS.md` would become an always-on rule for installers).
...
| Goal | Command |
|------|---------|
| Build JAR | `./gradlew jar` → `build/libs/gradle-tapi-mcp-server-0.14.0.jar` |
...
| Run MCP server | `GRADLE_PROJECT_DIR=/workspace java -jar build/libs/gradle-tapi-mcp-server-0.14.0.jar` |
