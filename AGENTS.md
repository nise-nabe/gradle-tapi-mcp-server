...
1. Downloads release JAR (version in `.cursor/install.sh` `GRADLE_TAPI_MCP_VERSION`, currently **0.13.1**) with SHA-256 verification to `~/.local/share/gradle-tapi-mcp-server/gradle-tapi-mcp-server.jar` so MCP can drive this repo's build when needed
...
| Goal | Command |
|------|---------|
| Build JAR | `./gradlew jar` → `build/libs/gradle-tapi-mcp-server-0.13.2.jar` |
...
| Run MCP server | `GRADLE_PROJECT_DIR=/workspace java -jar build/libs/gradle-tapi-mcp-server-0.13.2.jar` |
