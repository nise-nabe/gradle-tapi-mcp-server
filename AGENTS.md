...
- Stack: Kotlin 2.4.0, Java 17 toolchain, **Java** MCP SDK 2.0.0 (`io.modelcontextprotocol.sdk:mcp-bom`), Gradle Tooling API 9.6.1, `jackson-module-kotlin` 3.0.4 via `jsonMapper { addModule(kotlinModule()) }`. Official Kotlin MCP SDK (`io.modelcontextprotocol:kotlin-sdk`) exists but is not used; see `docs/kotlin-mcp-sdk-investigation.md`.
...
1. Downloads release **v0.2.3** JAR (SHA-256 verified) to `~/.local/share/gradle-tapi-mcp-server/gradle-tapi-mcp-server.jar` — **before** `./gradlew` so MCP can drive this repo's build
...
| Goal | Command |
|------|---------|
| Build JAR | `./gradlew jar` → `build/libs/gradle-tapi-mcp-server-0.2.3.jar` |
...
| Run MCP server | `GRADLE_PROJECT_DIR=/workspace java -jar build/libs/gradle-tapi-mcp-server-0.2.3.jar` |
