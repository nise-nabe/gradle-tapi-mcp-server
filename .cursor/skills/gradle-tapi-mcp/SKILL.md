...
The `gradle` MCP server (release v0.7.1) is configured in `.cursor/mcp.json`. `.cursor/install.sh`
...
5. Optional: `gradle_index_dependency_sources` then `gradle_search_dependency_sources` (or `gradle_search_dependency_sources_multi` for several names). `limit` / `perQueryLimit` omit = unlimited; `0` = empty (`per_query_limit` alias; `tokenMode=all` default)
