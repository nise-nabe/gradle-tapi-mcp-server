...
3. `gradle_get_project_overview` — project name and task counts (scope with `projectPath` on multi-module repos). Gradle 9.3+ may return `partial` + capped `failures` when some projects fail (`isError=false`)
