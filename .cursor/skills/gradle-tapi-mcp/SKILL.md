...
5. Optional: `gradle_index_dependency_sources` then `gradle_search_dependency_sources` (or `gradle_search_dependency_sources_multi` for several names), then `gradle_read_dependency_source` for a hit snippet (`line` + `contextLines=10` default; omit `line` for whole file). `limit` / `perQueryLimit` omit = unlimited; `0` = empty (`per_query_limit` alias; `tokenMode=all` default)
