...
The `gradle` MCP server (release v0.12.0) is configured in `.cursor/mcp.json`. `.cursor/install.sh`
...
3. `gradle_get_project_overview` — project name and task counts (scope with `projectPath` on multi-module repos). Gradle 9.4+ may return `partial` + capped `failures` when some projects fail (`isError=false`)
4. Optional: `gradle_get_dependency_resolution` — omit `configuration` to list resolvable/consumable names, or pass a resolvable `configuration` (and optional `dependency` filter) for resolved graphs without report tasks
5. `gradle_run_tasks` with `["build"]` or `gradle_run_tests` when verification is needed
6. Optional: `gradle_index_dependency_sources` then `gradle_search_dependency_sources` (or `gradle_search_dependency_sources_multi`), then `gradle_read_dependency_source`. Match `tokenMode` between index and search; on large repos prefer `background: true` + poll `gradle_get_dependency_sources_index_status`, or scope with `projectPath` / `artifacts[]` / `sourcePaths[]`, and `tokenMode: idents` for faster first indexes; use `downloadSources: true` with `artifacts[]` to fetch missing `*-sources.jar` (default Maven Central; set `sourcesRepositories` for corporate mirrors). Exact simple-name match only. Canonical workflow + JSON examples: repository `README.md` (**Dependency sources name locate**). `limit` / `perQueryLimit` omit = unlimited; `0` = empty (`per_query_limit` alias; `tokenMode=all` default). Idea keep-set is the **current project's** attached sources only. To inspect another version or a Maven GAV not in the graph (Kotlin / plugin / compiler), pass `artifacts[]` + `downloadSources: true` — do **not** list `gradleUserHome` / `~/.gradle/caches` / `wrapper/dists` / `jdks`. `sourcePaths[]` only when the local path is already known. Different Gradle **runtime**: `gradle_connect` `gradleVersion` then `gradle_get_build_environment` (do not switch the session Gradle merely to read sources).

Avoid `includeTasks=true` and heavy model queries unless necessary. On multi-module projects, pass `projectPath` (e.g. `:plugin`) to scope overview/model/invocation queries to a subproject subtree within the connected build. For included builds and `buildSrc`, pass `buildTreePath` (e.g. `:buildSrc`) after discovering it via `gradle_get_gradle_build`. `gradle_run_tasks` omits stdout/stderr by default (`includeOutput=false`).
