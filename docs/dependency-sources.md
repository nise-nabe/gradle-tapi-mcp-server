...
- `limit` / `perQueryLimit` are query-time only (no `formatVersion` bump): `0` = empty. Omit or null applies a server-side default cap of 500 to `limit` (`hitsTruncated` reports overflow); `perQueryLimit` stays unlimited when omitted. `2147483647` (`Int.MAX_VALUE`) = unlimited. `per_query_limit` is an alias for `perQueryLimit`.
