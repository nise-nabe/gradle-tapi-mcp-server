...
internal fun progressProperties(): Map<String, Any> =
    mapOf(
        ...
        "includeProblems" to booleanProperty(
            "Capture live Gradle Problems API events for new builds and include liveProblems (or merged terminal problems) in responses. Default false.",
        ),
    )
    ...
    listOf(
        ...
        tool(
            ...
            description = "Execute JVM tests with class, method, pattern, or task-scoped selection and return build outcome and summary. " +
                ...
                "Set includeProgress=true for detailed progress on foreground runs and includeProblems=true to capture live Gradle Problems API events.",
            schema = runOutputSchema(
                required = emptyList(),
                extraProperties = mapOf(
