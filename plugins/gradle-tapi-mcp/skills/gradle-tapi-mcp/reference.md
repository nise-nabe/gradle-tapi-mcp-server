...
Search hits may include `sourceRoot` from the index side-car `source-roots.tsv` (written at index time). `gradle_read_dependency_source` uses that path when `sourceRoot` is omitted (before falling back to cache jar lookup). `contextLines` max 100; `maxLines` max 2000.
