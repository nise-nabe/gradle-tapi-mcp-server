...
| `gradle_read_dependency_source` | 検索ヒットの `gav`+`path`（任意で `line`）から UTF-8 スニペット。`contextLines` 既定 10。`line` 省略は先頭から `maxLines`（既定 200）。Idea ディレクトリ / `sourcePaths` は `sourceRoot` 必須 |
