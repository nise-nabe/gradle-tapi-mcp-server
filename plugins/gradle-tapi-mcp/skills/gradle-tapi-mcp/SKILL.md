...
Gradle プロジェクトの**実行時状態**を MCP 経由で取得・実行する。宣言（`build.gradle.kts` 等）は `project-context-ingestion` を先に使い、MCP は解決済みバージョン・ビルド成否・タスク実行・ソース locate に使う。Idea keep-set 以外のバージョン（Kotlin / plugin など）は `artifacts[]` + `downloadSources` で索引し、`gradleUserHome` を直接読まない。
...
3. gradle_get_project_overview           # モジュール階層 + taskCount（軽量、projectPath / maxDepth / maxChildren 可）
...
| `gradle_connection_status` | 接続確認（キャッシュのみ。`runtimeStackAvailable=false` 時は `refresh=true` で実行時スタック取得。全接続一覧 + `refresh=true` は接続数ぶん fetch） |
...
`artifacts[]` はプロジェクトの解決グラフに載っている必要はない。サーバーが Maven local / 接続中プロジェクトの Gradle user home（`caches/modules-2/files-2.1`）/ MCP jars cache を内部参照し、無ければ `downloadSources` で取る。エージェントが `gradleUserHome` 配下を列挙しない。
...
`gradle_connect` と `gradle_get_build_cache_status`、およびモデル照会ツール（`gradle_get_project_overview` 等）は、同一 `projectDirectory` で MCP ビルド実行中は拒否される。
...
```json
{ "tasks": [":plugin:test"], "background": true }
```
...
{
  "testMethods": {
    "com.example.FooTest": ["shouldWork", "shouldFail"],
    "com.example.BarTest": ["testX"]
  }
}
```
...
**複数クラス／メソッドを 1 回で実行（同一プロジェクトの並列別呼び出しの代わり）**

```json
{
  "testMethods": {
    "com.example.FooTest": ["shouldWork"],
    "com.example.BarTest": ["testX", "testY"]
  }
}
```

→ `gradle_run_tests`（1 本の MCP ビルド。`background: true` なら `buildId` でポーリング）
