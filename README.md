...
The fat JAR is written to `build/libs/gradle-tapi-mcp-server-0.4.0.jar`.
...
```json
{
  "mcpServers": {
    "gradle": {
      ...
      "args": [
        "-jar",
        "/path/to/gradle-tapi-mcp-server/build/libs/gradle-tapi-mcp-server-0.4.0.jar"
      ],
      "env": {
...
| `gradle_connection_status` | Connection state for one project or all active connections (`connections[]`, `defaultProjectDirectory`). Cache-only by default; set `refresh: true` when `runtimeStackAvailable` is false and you need `gradleVersion` / `javaHome` |
