...
| `error.code: BUILD_ALREADY_RUNNING` | 同一 `projectDirectory` で既に MCP ビルド実行中。`gradle_get_build_status` で完了待ち、`gradle_cancel_build` で停止、または複数テストを 1 回の `gradle_run_tests` にまとめる（[テスト実行と並行性](#テスト実行と並行性)） |
