...
```
1. gradle_connection_status              # runtimeStackAvailable=true 時は gradleVersion / javaHome / javaVersion。false なら refresh=true または gradle_get_build_environment
...
| `gradle_connection_status` | 接続確認（キャッシュのみ。`runtimeStackAvailable=false` 時は `refresh=true` で実行時スタック取得） |
...
| `error.code: BUILD_ALREADY_RUNNING` | 同一 `projectDirectory` で既に MCP ビルド実行中、またはモデル照会とビルドの競合。`gradle_get_build_status` で完了待ち、`gradle_cancel_build` で停止、または複数テストを 1 回の `gradle_run_tests` にまとめる（[テスト実行と並行性](#テスト実行と並行性)） |
